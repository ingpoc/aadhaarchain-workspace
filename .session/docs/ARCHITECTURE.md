...
AgentGuard across ONDC Buyer and ONDC Seller. Build order lives in
`IMPLEMENTATIONPLAN.md`; verification in
`../../.agents/skills/testing-ledger/SKILL.md`; product scope in `PRODUCTIDEA.md`.
This document owns stable architecture only. Current implementation and launch
status lives exclusively in `../checklist/checklist.json`.
...
| --- | --- |
| AgentGuard policy service | Registration, mandates, evaluation, approval, pause, receipts |
| Mandate editor | User edits allowed actions and auto-approve limits; compile + confirm |
| Shared tool runner | Host-agnostic tools: navigate, search, cart, checkout, publish, refund |
| Agent host (text) | Cursor agent runtime for Buyer/Seller |
| Agent host (voice) | Realtime model over WebRTC using the same tools |
| Buyer application | Discovery, cart, guarded checkout, orders and issues |
| Seller application | Catalog, orders, fulfilment and guarded refunds |
| CommerceV1 | Carts, quotes, inventory reservations, orders, payments, ledger and refunds |
| Commerce compatibility adapter | Preserve legacy Buyer/Seller request shapes without creating another store |
| ONDC protocol adapter | Registry, signed Beckn requests, callbacks and recovery |
| Payment adapter | Execute and reconcile payment after Buyer authorization |
| Durable state | Policy, nonce, approvals, commerce, receipts and ONDC envelopes |
| Host identity adapter | Resolve an authenticated principal without becoming the authorization owner |
