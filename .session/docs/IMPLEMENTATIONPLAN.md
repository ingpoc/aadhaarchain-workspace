...
`ARCHITECTURE.md`. A new agent should read `../../AGENTS.md` for routing, then only
the owners relevant to the selected milestone and the testing-ledger skill.
...
status is recorded in `../checklist/checklist.json`. If repository reality contradicts
this plan, stop, update the owning contract with evidence, and then implement;
...
This section explains the baseline the plan evolves; it does not own current
readiness. Use `../checklist/checklist.json` for status.
...
1. Run the baseline commands in `.agents/skills/testing-ledger/SKILL.md`.
...
- The testing-ledger demo gate passes twice consecutively.
...
| --- | --- | --- |
| P0 Auth0 social | **PreProd/FQDN pass; production policy partial** | Authorization Code Flow and Buyer/Seller FQDN acceptance passed; production tenant policy, rotation, step-up, role/resource isolation and session/device controls remain |
| P1 Demo auth gated | **partial** | Staging/prod force-off demo-continue and simulated labels are implemented; retain partial until the direct current regression receipt is linked here |
| P2 Portal and participant identity | **partial** | Dedicated LOG10 LBNP PreProd registration exists; the captured portal profile remains `Integration in Progress`. Required production roles need a fresh portal readback and operator decision |
...
| P4 PSP/logistics/IGM | **partial; external LSP blocked** | The official LOG10 Workbench forward lifecycle passed and CommerceV1 correlation is implemented. TapTap `on_init` omitted the mandatory rider-check field, so external confirm/update/status/track did not run. PSP, IGM, settlement and full Retail lifecycle remain open. |
| P5 Commerce demo mode | **PreProd complete; production pending** | `VITE_COMMERCE_DEMO_MODE=false` is deployed and the evidence gate is unlocked for PreProd. This does not enable real payment or production ONDC |
...
- commands from `.agents/skills/testing-ledger/SKILL.md` it must run;
...
This table records implementation milestone history only. It is not current
product readiness; `../checklist/checklist.json` is the sole status owner.
...
Customer-first product and launch status is owned only by `../checklist/checklist.json`.
This plan retains sequencing and historical closure evidence; it must not be
parsed as a second checklist status source.
