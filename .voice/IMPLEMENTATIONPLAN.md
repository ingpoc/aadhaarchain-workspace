...
| P3 BAP adapter | **PreProd partial; current external gate** | Registry, signing and public callback routes are configured, but the 2026-07-24 frozen Buyer search was ACKed without a correlated `on_search`. Obtain PreProd gateway fanout/delivery diagnostics, then repeat one frozen-source signed retail search and require the correlated inbox record plus Buyer results identified as `ondc-network` before `select` / `init` / `confirm`. Evidence owner: `.cursor/skills/ondc-testing/references/matrix-status.md` and `evidence/preprod-buyer-search-20260724-032417.json`. |
