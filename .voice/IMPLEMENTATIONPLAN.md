...
| P3 BAP adapter | **PreProd partial; current external gate** | Registry, signing, public callbacks, PostgreSQL durability, signed delivery, and correlated `on_search` were proved in the 2026-07-24 retest. The Seller had zero published items, so the callback catalog and Buyer `ondc-network` results were empty. Publish one explicitly approved Seller item, then repeat one frozen-source search and require a non-empty correlated callback before `select` / `init` / `confirm`. Evidence owner: `.agents/skills/testing-ledger/references/matrix-status.md` and `evidence/preprod-postgres-search-20260724-135337.json`. |
| P3a Logistics decision gate | **required before P4** | Define the active logistics domain scope, protocol version, and Buyer/BPP/Logistics Provider role ownership before lifecycle-contract implementation. |
